...
class HailoWhisperPipeline:
    ...
    def __init__(self, encoder_model_path: str, decoder_model_path: str, variant="tiny", host="arm64", multi_process_service=False):
        ...
        :param multi_process_service: Whether to enable multi-process service
        """
        ...
        print(f"Using Whisper {self.variant} model with decoding sequence length: {self.decoding_sequence_length}")
        ...
        # Print multi-process status
        if self.multi_process_service:
            print("Multi-process service enabled")
                ...
                with decoder_infer_model.configure() as decoder_configured_infer_model:
                    ...
                    while self.running:
                        try:
                            ...
                            transcriptions = []
                            # Zero-copy optimization: Use ascontiguousarray only when necessary
                            if not input_mel.flags.c_contiguous:
                                input_mel = np.ascontiguousarray(input_mel)
                            
                            # Time the encoder processing
                            encoder_start_time = time.time()
                            encoder_bindings.input().set_buffer(input_mel)
                            # Pre-allocate buffer for better performance
                            if not hasattr(self, '_encoder_output_buffer'):
                                self._encoder_output_buffer = np.zeros(encoder_infer_model.output().shape).astype(np.float32)
                            encoder_bindings.output().set_buffer(self._encoder_output_buffer)
                            ...
                            # Pre-allocate decoder input IDs for better performance
                            if not hasattr(self, '_decoder_input_ids'):
                                self._decoder_input_ids = np.zeros((1, self.decoding_sequence_length), dtype=np.int64)
                            self._decoder_input_ids[0, 0] = start_token_id[0]

                            generated_tokens = []
                            decoder_outputs = None
                            
                            # Time the decoder processing
                            decoder_start_time = time.time()
                            # Run Decoder Iteratively
                            for i in range(self.decoding_sequence_length - 1):
                                tokenized_ids = self._tokenization(self._decoder_input_ids)
                                ...
                                if not hasattr(self, '_decoder_buffers'):
                                    self._decoder_buffers = [
                                        np.zeros(decoder_infer_model.output(name).shape).astype(np.float32) for name in sorted_output_names
                                    ]
                                
                                for name, buffer in zip(sorted_output_names, self._decoder_buffers):
                            ...
                            # Note: We'll control timing display from the main app
